...
inline constexpr std::uint32_t fnv1a(char const* str, std::size_t size) {
  std::uint32_t hash = fnv_offset<std::uint32_t>;
...
LIBREPR_FORCE_INLINE constexpr std::uint32_t fnv1a(std::string_view str) {
  return fnv1a(str.begin(), str.size());
}

template <std::size_t N>
LIBREPR_FORCE_INLINE constexpr std::uint32_t fnv1a(char const (&str)[N]) {
  return fnv1a(str, N - 1);
}

LIBREPR_FORCE_INLINE constexpr std::uint32_t operator""_fnv1a(const char* str, std::size_t size) {
  return fnv1a(str, size);
}

namespace detail {
struct Hash {
  // Everything being implicit here is intended. This type is just a helper.
  //NOLINTBEGIN
