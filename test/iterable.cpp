...
TEST(iterable, array) {
  EXPECT_EQ(repr((int(&&)[3]){1, 2, 3}), "(int(&&)[3]){1, 2, 3}");
  EXPECT_EQ(repr((char const*(&&)[3]){"1", "2", "3"}), 
                 "(char const*(&&)[3]){\"1\", \"2\", \"3\"}");
...
TEST(iterable, map) {
  EXPECT_EQ(repr(std::map<int, int>{{1, 6}, {2, 5}, {3, 4}}), 
            "std::map<int, int>{{1, 6}, {2, 5}, {3, 4}}");
}
